...
public final class SDK {
	 ...
	 * @throws IOException 
	 */
	public static byte[] unzip(byte[] ziped) throws JavaScriptException {
		Int8Array i8ArrZiped = Int8ArrayNative.create(ziped.length);
		i8ArrZiped.set(ziped);
		Int8Array i8ArrUnZiped = _unzip(Uint8ArrayNative.create(i8ArrZiped.buffer()));
		byte[] ret = new byte[i8ArrUnZiped.length()];
		for (int i = 0; i < ret.length; ++i) {
			ret[i] = i8ArrUnZiped.get(i);
		}
		return ret;
	}
	private static native Int8Array _unzip(Uint8Array compressed)/*-{
															var ret = new $wnd.Zlib.Inflate(compressed).decompress();
